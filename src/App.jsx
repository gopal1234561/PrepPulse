...
const enableNotifications=async()=>{try{if(!("Notification"in window)){setToast("This browser does not support notifications.");return}const p=await Notification.requestPermission();if(p!=="granted"){setToast("Notification permission was not granted.");return}const sub=await registerPush();await fetch(API+"/api/subscribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({subscription:sub,times})}).then(async r=>{if(!r.ok)throw new Error((await r.json()).error||"Could not connect to push server.")});setNotifications(true);const reg=await navigator.serviceWorker.ready;await reg.showNotification("PrepPulse",{body:"Mobile push reminders are connected.",icon:"/icon.svg",badge:"/icon.svg",tag:"preppulse-connected"});setToast("Push reminders connected successfully.")}catch(e){setToast(e.message||"Could not enable push reminders.")}};
